...
namespace stm32plus {
    ...
    inline bool GraphicsLibrary<TDevice,TDeviceAccessMode>::drawBitmap(const Rectangle& rect,InputStream& source) {
      ...
      return retval;
		 ...
		 * @param rect The size and position of the rectangle on the display.
		 * @param source The input stream supplying the source data
		 * @param dmaChannel The DMA channel to use for memory-to-memory copying to the display, or NULL to use the CPU.
		 * @return false if there was a failure (e.g. stream failure)
		 */

		template<class TDevice,typename TDeviceAccessMode>
    inline bool GraphicsLibrary<TDevice,TDeviceAccessMode>::drawBitmap(const Rectangle& rect,InputStream& source,DMA_Channel_TypeDef *dmaChannel) {

      int16_t vpos;
      uint8_t *evenLines,*oddLines,*buffer;
      uint32_t actuallyRead,bytesPerPixel;
      DmaFsmcLcdMemoryCopy *dma;
      bool retval;
      ...
      // create the DMA transfer object

      dma=this->createDmaMemoryCopy(dmaChannel);
      ...
      for(vpos=0;vpos<rect.Height;vpos++) {
        ...
        dma->start(buffer,rect.Width*bytesPerPixel);
      }
