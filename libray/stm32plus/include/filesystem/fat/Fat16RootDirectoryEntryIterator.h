...
namespace stm32plus {
     ...
     */
        ...
        uint32_t _rootDirMaxEntries;
            ...
            protected:
                uint32_t entryOffsetInSector(uint32_t entryIndex_) const;
                bool isEntryInCurrentSector() const;
                uint32_t sectorIndexForEntry(uint32_t entryIndex_) const;
                bool readSectorForEntry(uint32_t entryIndex_);
