...
namespace stm32plus {
    ...
    namespace display {
        ...
        ADS7843AsyncTouchScreen::ADS7843AsyncTouchScreen(
                ...
                const Gpio& irqPin,
                ExtiBase& exti) :
                        TouchScreen(calibration,postProcessor),
                        _spiWriter(spi),
                        _irqPin(irqPin) {

            // register ourselves as an observer of the EXTI events

            _linkManager.setLink(exti.insertObserver(*this));
        }
        ...
        void ADS7843AsyncTouchScreen::getSamples(uint16_t *xvalues,uint16_t *yvalues,uint16_t sampleCount) const {
            ...
            _spiWriter.getPeripheral().setNss(false);

            // get the X values and don't power down afterwards

            getSamples(ChannelX | START | PD0,ChannelX | START | PD0,xvalues,sampleCount);

            // get the Y samples and power down afterwards

            getSamples(ChannelY | START | PD0,ChannelY | START,yvalues,sampleCount);

            // set CS = high

            _spiWriter.getPeripheral().setNss(true);
        }
        ...
        void ADS7843AsyncTouchScreen::getSamples(uint8_t firstCommand,uint8_t lastCommand,uint16_t *values,uint16_t sampleCount) const {
            ...
            _spiWriter.write(&firstCommand,1);
            _spiWriter.write(&dummy,1);
            _spiWriter.write(&dummy,1);
            ...
            while(sampleCount--!=1) {
                ...
                _spiWriter.write(&dummy,1,&data[0]);
                _spiWriter.write(&dummy,1,&data[1]);
            ...
            _spiWriter.write(&lastCommand,1);
        ...
        void ADS7843AsyncTouchScreen::onNotify(
                Observable& sender_ __attribute((unused)),
                ObservableEvent::E event_,
                void *context_ __attribute((unused))) {

            // check for correct event and notify observers that sampling is now possible

            if(event_==ObservableEvent::EXTI_Triggered)
                notifyObservers(ObservableEvent::TouchPanel_ReadyForSampling);
        }


        /**
         * Exti has been destroyed, unregistering will be impossible
         */

        void ADS7843AsyncTouchScreen::onSubjectDestroyed() {
            _linkManager.onSubjectDestroyed();
        }
    }
}
