...
namespace stm32plus {
    ...
    Spi3::Spi3(
            ...
            uint16_t cpol,
            uint16_t cpha,
            uint16_t polynomial) : SpiPeripheral(SPI3,DMA2_Channel2,DMA1_Channel1,2,SPI3_IRQn) {
        ...
        if(mode == SPI_Mode_Master) {
            pb.initialise(GPIO_Speed_50MHz,GPIO_Mode_AF_PP,GPIO_Pin_SCK | GPIO_Pin_MOSI);
            pb.initialise(GPIO_Speed_50MHz,GPIO_Mode_IN_FLOATING,GPIO_Pin_MISO);
        } else {
            pb.initialise(GPIO_Speed_50MHz,GPIO_Mode_IN_FLOATING,GPIO_Pin_SCK | GPIO_Pin_MOSI);
            pb.initialise(GPIO_Speed_50MHz,GPIO_Mode_AF_PP,GPIO_Pin_MISO);
        }
